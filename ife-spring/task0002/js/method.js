...
function cloneObject(src){
	...
	if(typeof(src)=="object"){
		if (Object.prototype.toString.call(src)==='[Object Date]') {//判断这个是不一个Date对象
			clone=src;
		}
		else{
			if (Object.prototype.toString.call(src)==='[Object Array]') {//这个是不一个数组对象,如果是就就是初始化一下这个值，如果不是就给它初始化一个函数对象
		...
		for(var i in src){
			 if (src.hasOwnProperty(i)) {//用来排除继承属性
...
function addClass(element, newClassName) {
  ...
  element.className=(element.className==="")?element.className:element.className+" "+newClassName;
  // 等价于下面：
...
// AJax-向服务器发送请求,当async=true时，要在onreadystatechange()方法中执行相应的函数
  xmlhttprequest.onreadystatechange=function(){
    if(xmlhttprequest.readyState==4 && xmlhttprequest.status==200){
      document.write("hello this is the Ajax first procedure");
    }

  }
xmlhttprequest.open("GET","/ajax/test1.txt",true);
xmlhttprequest.send();
