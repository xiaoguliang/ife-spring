...
function clickMe(){  
    content=text.value;//取文本框的内容
    content=trim(content);//对字符串头尾进行空格字符的去除、包括全角半角空格、Tab等
   ...
   // console.log(arr.length);
    ...
    if (arr.length>10 ) {//当爱好数量超过10个时
     warn.style.display="block";//显示红色的错误提示文字
     warn.innerHTML="最多写十个"
    }
    else if(arr[0]==""){//去除第一个为空情况
        warn.style.display="block";
    warn.innerHTML="最少写一个"
    }
     else if(arr.length===0){//当输入的爱好数量为零时
    warn.style.display="block";
    warn.innerHTML="最少写一个"
     }
     else {
        warn.style.display="none";var checkboxStr = "";
